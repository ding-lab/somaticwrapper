...
$green [0] Generate bams if input files are fastqs + Submit all steps with deps
$green [1] Run Mutect2
$green [2] Run filter Mutect2 result
$green [3] Run parse Mutect2 result
$cyan [4] Generate per-sample MAF
$cyan [5] Generate merged run-level report
$normal
...
if (! -d $HOME1."/tmpsomatic") { `mkdir $HOME1"/tmpsomatic"`; }
my $job_files_dir = $HOME1."/tmpsomatic";

#store LSF output and error files here
if (! -d $HOME1."/LSF_DIR_SOMATIC") { `mkdir $HOME1"/LSF_DIR_SOMATIC"`; }
...
### running tools / DB paths (as provided) ##
my $MINLEN=50;
my $MIN_VAF=0.02;
my $MIN_DEP=8;
my $MIN_MUT=3;
...
my $bamreadcount="/storage1/fs1/dinglab/Active/bam-readcount/0.7.4/bam-readcount";
...
sub bsub_mutect2{
    my ($step_by_step) = @_;
    # reset per-sample deps before submitting this sample’s chain
    @J1_JIDS = ();
    $J2_JID  = '';
    $J3_JID  = '';
...
sub bsub_filter_mutect2 {
    my ($step_by_step) = @_;
    if ($step_by_step) { $hold_job_file = ""; } else { $hold_job_file = $current_job_file; }
...
sub bsub_vcf_2_maf{
    ...
    print MAF "rm -f \${F_VCF_2}\n";
    print MAF "rm -f \${F_VEP_2}\n";
    ...
    $bsub_com = "LSF_DOCKER_ENTRYPOINT=/bin/bash LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub ${dep3}-g /$compute_username/$group_name -n 1 -R \"select[mem>30000] rusage[mem=30000]\" -M 30000000 -a 'docker(ensemblorg/ensembl-vep:release_102.0)' -o $lsf_out -e $lsf_err bash $sh_file\n";
    print $bsub_com;

    my $bsub_out = `$bsub_com`;
    if ($bsub_out =~ /Job <(\d+)>/) {
        my $j4_jid = $1;
        push @ALL_J4_JIDS, $j4_jid;
    }
}

sub bsub_run_report {
    # Depends on all j4 jobs across all samples
    my $working_name = (split(/\//,$run_dir))[-1];
    ...
    my $cmd = "bsub ${dep_all}-q $q_name -g /$compute_username/$group_name -n 1 -R \"select[mem>30000] rusage[mem=30000]\" -M 30000000 -a 'docker(scao/dailybox)' -o $lsf_out -e $lsf_err bash $sh_file\n";
    print $cmd;
    my $bsub_out = `$cmd`;
    if ($bsub_out =~ /Job <(\d+)>/) { $J5_JID = $1; }
}

# ---- submission orchestrator (new j0) ----
sub bsub_submit_all {
    # For each sample (outer loop in main), this gets called once.
    # It submits j1 (per-chr) -> j2 (filter) -> j3 (parse) -> j4 (vcf2maf),
    # wiring dependencies via -w done(<JID>).
    bsub_mutect2(0);        # fills @J1_JIDS
    bsub_filter_mutect2(0); # waits for all j1s
    bsub_parse_mutect2(0);  # waits for j2
    bsub_vcf_2_maf(0);      # waits for j3; collects J4 job IDs globally
    print "Submitted: ", scalar(@J1_JIDS), " j1 jobs; j2 JID=$J2_JID; j3 JID=$J3_JID\n";
}

# =================== MAIN ===================

# begin to process each sample for steps 0-4 (sample-level)
if ($step_number < 5) {
    ...
    # After queuing all samples in step 0, enqueue run-level job j5
    if ($step_number == 0) {
        bsub_run_report();      # j5 waits on ALL j4 jobs
        print "Queued run-level job: j5=$J5_JID\n";
    }
}

# Run-level step 5: generate report (manual run)
if($step_number==5) {
