...
INDEL_RAW="/usr/local/somaticwrapper/docker/demo/results/varscan/varscan_out/varscan.out.som_indel.vcf"
SNV_RAW="/usr/local/somaticwrapper/docker/demo/results/varscan/varscan_out/varscan.out.som_snv.vcf"

# try to have all output go to output_dir
mkdir -p $OUTPUT_DIR

ARGS="--varscan_indel_raw $INDEL_RAW \
--varscan_snv_raw $SNV_RAW \
--dbsnp_db $DBSNP_DB \
--varscan_config $VARSCAN_CONFIG "

perl ../../SomaticWrapper.pl $ARGS --results_dir $OUTPUT_DIR 4
