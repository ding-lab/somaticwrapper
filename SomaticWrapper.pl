...
require("src/caller_vcf_filter.pl");
...
* [caller_filter]  Filter VCF based on variant length, VAF, and read depth
...
7 parse_pindel:
    ...
    --reference_fasta s: path to reference.  Required
    --no_delete_temp : if defined, do not delete temp files
    --bypass: Bypass AF/Classification filter by retaining all reads
8 merge_vcf:
...
caller_filter: apply VAF, indel length, and read depth filters to a VCF
    --input_vcf s: VCF file to process.  Required
    --output_vcf s: Name of output VCF file (written to results_dir/caller_vcf_filter/output_vcf).  Required.
...
if ($bypass) {
    print STDERR "Filter bypass retains all reads\n";
...
if (($step_number eq '1') || ($step_number eq 'run_strelka')) {
    ...
    die("reference_fasta undefined \n") unless $reference_fasta;
    parse_pindel($results_dir, $job_files_dir, $reference_fasta, $perl, $gvip_dir, $filter_dir, $pindel_dir, $pindel_config, $pindel_raw, $no_delete_temp, $bypass);
...
} elsif ($step_number eq 'caller_filter') {
    die("input_vcf undefined \n") unless $input_vcf;
    die("output_vcf undefined \n") unless $output_vcf;
    die("caller undefined \n") unless $caller;
    die("vcf_filter_config undefined \n") unless $vcf_filter_config;
    caller_vcf_filter($results_dir, $job_files_dir, $filter_dir, $input_vcf, $output_vcf, $caller, $vcf_filter_config, $bypass);
